...
- [ ] Browser verification: Use agent-browser to walk through the affected flow end-to-end.
  ...
  - If the change introduces visible new behavior, capture 1–3 screenshots that clearly demonstrate it and retain them locally for the pull request's `Screenshots` section. Do not post implementation screenshots or browser-verification notes to the GitHub issue. The `pull-request` skill owns attaching those screenshots when the PR is opened.
